...
func (r *postgresMeetingRepository) UpdateEmbedding(ctx context.Context, telegramID int64, fileId string, embedding []float32) error {
	tx, err := r.db.BeginTxx(ctx, nil)
	if err != nil {
		return fmt.Errorf("failed to begin transaction: %w", err)
	}
...
func (r *postgresMeetingRepository) SearchByEmbedding(ctx context.Context, telegramID int64, queryEmbedding []float32, limit, offset int) ([]models.Meeting, error) {
	var meetings []models.Meeting
