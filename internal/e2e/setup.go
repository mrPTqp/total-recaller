...
import (
	...
	"github.com/mrPTqp/total-recaller/internal/e2e/llmmock"
	"github.com/mrPTqp/total-recaller/internal/e2e/telegrammock"
	"github.com/mrPTqp/total-recaller/internal/e2e/tokenmock"
	"github.com/mrPTqp/total-recaller/internal/e2e/transcribermock"
	"github.com/stretchr/testify/require"
	"go.uber.org/zap"
)

// TestMain is the entry point for all tests in this package.
// It manages the lifecycle of the shared PostgreSQL container.
func TestMain(m *testing.M) {
	// Create shared PostgreSQL container before running tests
	ctx := context.Background()
	shared, err := getSharedPostgres(ctx)
	if err != nil {
		fmt.Printf("❌ Failed to create shared PostgreSQL container: %v\n", err)
		os.Exit(1)
	}
	...
	// Cleanup: stop the shared container after all tests
	if shared != nil {
		if stopErr := shared.stop(ctx); stopErr != nil {
			fmt.Printf("Warning: failed to stop shared PostgreSQL container: %v\n", stopErr)
		}
	}
